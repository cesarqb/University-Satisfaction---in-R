...
write.csv(predict_fase_1,'xgb_2_fase_1_20170727.csv',row.names = F)

setwd('C:/Users/Cesar Quezada/Dropbox/KAGGLE/01. Concurso 01/02_segunda_fase')
write.csv(data_subir,'xgb_2_fase_2_20170727.csv',row.names = F)

# library(ModelMetrics)
# mlogLoss(target_cv, predicted = as.matrix(predict_fase_1[,!names(predict_fase_1)%in%c(id,target)]))

mlogloss_1 <- function(label, pred){
  ...
  label <- label_mat
  #I tried to multiply label sparse matrix with prediction matrix, but it returned an error, like feval() something.
