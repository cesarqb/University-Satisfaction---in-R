...
#==================================================================
# Glove
prep_fun_1 <- text.clean
tok_fun_1 <- tokenize_word_stems

it_train_1 <- itoken(datos_txt$COMENTARIO,
                     preprocessor = prep_fun_1,
                     tokenizer = tok_fun_1,
                     ids = datos_txt[,id],
                     progressbar = T,
                     language = "spanish")
...
vocab_11 <- prune_vocabulary(vocab_1)

vectorizer_1 <- vocab_vectorizer(vocab_11)
...
dtm_train_tfidf_1 <- fit_transform(dtm_train_1, tfidf_1)
class(dtm_train_tfidf_1)
dim(dtm_train_tfidf_1)

set.seed(12345)
vectorizer_2 <- vocab_vectorizer(vocab_11,
                                 grow_dtm = F,
                                 skip_grams_window = 10L)
tcm_1 <- create_tcm(it_train_1, vectorizer_2)
...
glove <- GlobalVectors$new(word_vectors_size = 50,
                           ...
                           lambda = 1e-5)
set.seed(12345)
glove$fit(tcm_1, n_iter = 100)

word_vectors <- glove$get_word_vectors()
prueba <- word_vectors['merecen',,drop=FALSE]

distancias <- sim2(x = word_vectors,y=prueba,method='cosine',norm='l2')
head(sort(distancias[,1],decreasing = T),5)

# Transformando en matriz
cantidada <- apply(dtm_train_tfidf_1,1,function(x)sum(x!=0))

nombres_col_tfidf <- colnames(dtm_train_tfidf_1)

word_vectors_1 <- word_vectors[nombres_col_tfidf,,drop=FALSE]

dtm_train_tfidf_1_word2vec <- dtm_train_tfidf_1%*%word_vectors_1

dtm_train_tfidf_1_word2vec <- dtm_train_tfidf_1_word2vec/cantidada
dtm_train_tfidf_1_word2vec <- data.frame(as.matrix(dtm_train_tfidf_1_word2vec))
names(dtm_train_tfidf_1_word2vec) <- paste('word2vec',seq(1:50),sep='_')
...
vocab1 <- prune_vocabulary(vocab,
                           term_count_min = 10, # Lo importante es el doc_proportion_min
...
# LSA
set.seed(12345)
lsa <- LatentSemanticAnalysis$new(n_topics = 20)
lsa_data <- lsa$fit_transform(dtm_train_tfidf)
for(i in 1:20){
  boxplot(lsa_data[,i]~datos_txt$NPS)
}
lsa_data <- data.frame(lsa_data)
names(lsa_data) <- paste('lsa',seq(1:20),sep='_')
...
doc_topic_distr <- 
  lda_model$fit_transform(dtm, n_iter = 10000, convergence_tol = 0.001,
...
doc_topic_distr <- data.frame(doc_topic_distr)
names(doc_topic_distr) <- paste('lda',seq(1:10),sep='_')
...
set.seed(12345)
tsne <- Rtsne(data_1, dims = 2, perplexity = 30, verbose = TRUE, 
              max_iter = 1000)
...
lista_subir <- list(datos_modeliza = datos_modeliza,
                    ...
                    id = id,
                    target = target)
saveRDS(lista_subir, "lista_subir_v_final_20170802.rds")
#===============================================================================
